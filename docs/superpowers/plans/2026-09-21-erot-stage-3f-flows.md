...
- [ ] Implement callable JAX-compatible entropy, linear potential and quadratic
...
- [ ] Implement entropy prox through a stable scalar equation in log space,
...
- [ ] Test scalar optimality against independent SciPy roots for large positive,
  negative and small-alpha inputs, plus prox/gradient/objective conventions.
- [ ] Keep base dependencies JAX and NumPy; no Flax or Optax required.

## Task 2: one correct unregularized PDHG JKO step

- [ ] Define full PDHG state (coupling, density, both duals, extrapolated
  iterates and work count). Solve dt*F(rho)+0.5*<C,pi>, pi>=0,
  rows(pi)=rho, cols(pi)=previous. Honor theta and both step sizes.
- [ ] Use the actual coupled constraint operator K(pi,rho)=(rows(pi)-rho,
  cols(pi)); prove/bound its norm, and require tau*sigma*||K||²<1.
- [ ] Convergence includes both equality residuals, positivity, finite values
...
- [ ] Compare quadratic and entropy JKO cases to independently stated CVXPY
...
- [ ] Implement constrained density optimization of F(rho)+T_epsilon(rho,prev)
  /(2*dt). The transport derivative is f/(2*dt), fixing the audited factor two.
- [ ] Use a documented projected or mirror gradient algorithm with explicit
...
- [ ] Reuse cost-unit potentials between outer iterations. An unsuccessful OT
...
- [ ] Match the finite-epsilon objective to a joint-coupling CVXPY reference.
...
- [ ] Add FlowState with current mass, accepted count, physical time and backend
  continuation state. Separate snapshots from full solver checkpoints.
- [ ] Provide jko_step and a fixed-length chunk runner with explicit snapshot
...
- [ ] Test interruption-ready state structure, no time advance on inner failure,
  mass conservation, entropy/quadratic reference trajectories and scan use.
- [ ] Add runnable heat-flow and quadratic-flow examples. State grid, boundary
...
- [ ] Update provenance, public API guidance and numerical limitations.
- [ ] Run full CPU suite, new reference tests, CLI/install checks as needed and
  Ruff. Prepare GPU checks without claiming unavailable hardware results.
- [ ] Commit important increments; request the execution skill's final review,
